...
export async function POST(
    ...
    try {
        ...
        if (project.invoices.some(i => i.type === "full_payment" && i.paymentLink)) {
            return NextResponse.json({ error: "Cannot add items. An invoice has already been generated." }, { status: 403 });
        }
        ...
        return NextResponse.json({ item: newItem, projectTotal: updatedProject.totalPrice }, { status: 201 });
    } catch (error) {

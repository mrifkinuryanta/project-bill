...
const MAYAR_API_BASE_URL = process.env.NEXT_PUBLIC_MAYAR_IS_PRODUCTION === 'true'
    ? 'https://api.mayar.id/hl/v1'
    : 'https://api.mayar.id/hl/v1'; // Mayar essentially uses the same URL but relies on the API key (Live/Test) for environment distinction.
...
export async function POST(
    ...
    try {
        ...
        if (!MAYAR_API_KEY) {
            throw new Error("Mayar API Key is not configured in environment variables.");
        }
        ...
        if (invoice.project.currency !== "IDR") {
            return NextResponse.json({ error: "Mayar currently only supports IDR" }, { status: 400 });
        }
        ...
        const payload = {
            ...
            expiredAt: invoice.dueDate ? new Date(invoice.dueDate).toISOString() : undefined,
        };
        ...
        if (!mayarRes.ok) {
            ...
            throw new Error(`Failed to create Mayar payment link: ${errorData.message || mayarRes.statusText}`);
        }
